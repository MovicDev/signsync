...
// Store letter sequence for sentence building
...
export default function SignLanguageCamera() {
  ...
  const startPredictionLoop = () => {
    ...
    // Capture and predict every 800ms for smoother real-time experience
      ...
      if (response.data.success) {
        ...
        if (conf > 0.7) {
          ...
          setLetterHistory(prev => {
            ...
            if (newHistory.length > MAX_SEQUENCE_LENGTH) {
              newHistory.shift();
            }
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.instructions}>
        ...
        <Text style={[styles.instructionText, { marginTop: 5, fontSize: 12 }]}>
          {isActive ? '📸 Capturing automatically every 0.8 seconds' : '⏸️ Recognition paused'}
        </Text>
      </View>
    </SafeAreaView>
  );
